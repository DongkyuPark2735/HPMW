...
INSERT INTO EVENTS(EVNO, EVTITLE, EVDETAIL, EVSTARTDATE, ETNO, MNO)
    VALUES(TO_CHAR(SYSDATE, 'RRMMDD') ||TRIM(TO_CHAR(EVENTS_SEQ.NEXTVAL, '009')), 'KYUNGHEEUC', '기업행사', 
             TO_DATE('2022/07/07', 'YYYY/MM/DD'), 40, 1811009);
commit;
...
-- 지난 행사 목록 데이터 
SELECT * FROM PASTPARTTIMERCONTRACTDATA;
SELECT * 
    FROM (SELECT ROWNUM RN, A.*
                 FROM (SELECT *
...
DROP SEQUENCE PASTPARTTIMERCONTRACTDATA_SQ;
CREATE SEQUENCE PASTPARTTIMERCONTRACTDATA_SQ NOCACHE NOCYCLE;
                        
INSERT INTO PASTPARTTIMERCONTRACTDATA(PCDATANO, PTCONNO, PTID, PTNAME, PTTEL, PTEMAIL, PTADDRESS, BTNO, PTACCOUNTNO, EVNO, PTSTATUS, PTWORKTIME, PTHOURLYWAGE, PTTOTALPAY, PTRDATE)
    VALUES(PASTPARTTIMERCONTRACTDATA_SQ.NEXTVAL, '00000019', '1234', '한만운', '010-1111-5555', 'HONG@NAVER.COM', '서울시 강남구 압구정', 300, '55444466664848', '220627004', 1,  15, 9000, 135000, '22/07/05');
                        
-- 지난 행사 글 갯수
select COUNT(*) from PASTPARTTIMERCONTRACTDATA;
